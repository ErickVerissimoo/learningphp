...
throw new \Exception('Tarefa com mesmo nome já cadastrada');
        ...
        $sql = "INSERT INTO tarefa (name, description, scheduledAt) VALUES (?, ?, ?)";
        $stmt = $this->mysqli->prepare($sql);
        $data =  $task -> __get('scheduledAt');
        $instance = DateTime::createFromInterface($data);
        $stmt->bind_param(
            "sss",
            $task->name,
            $task->description,
            $instance -> format(DateTime::ATOM)
        );
        $stmt->execute();
        $stmt->close();
    }
    ...
    private function exists(string $name): bool{
        $sql = "SELECT COUNT(*) FROM tarefa WHERE name =?";
        $stmt = $this->mysqli->prepare($sql);
